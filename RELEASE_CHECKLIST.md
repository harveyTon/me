...
Version strategy: current release is `v0.2.1` and follows semantic versioning.

## Before Tagging

- Confirm `Cargo.toml` version is `0.2.1`.
...
```bash
tmpdir="$(mktemp -d)"
curl -L -o "${tmpdir}/me.tar.gz" https://github.com/harveyTon/me/releases/download/v0.2.1/me-v0.2.1-linux-x64.tar.gz
...
```bash
tmpdir="$(mktemp -d)"
curl -L -o "${tmpdir}/me.tar.gz" https://github.com/harveyTon/me/releases/download/v0.2.1/me-v0.2.1-macos-arm64.tar.gz
tar -C "${tmpdir}" -xzf "${tmpdir}/me.tar.gz"
"${tmpdir}/me" --help
```
...
```powershell
$tmpdir = New-Item -ItemType Directory -Force "$env:TEMP\me-v0.2.1"
Invoke-WebRequest -Uri https://github.com/harveyTon/me/releases/download/v0.2.1/me-v0.2.1-windows-x64.zip -OutFile "$tmpdir\me.zip"
...
```bash
cargo install --git https://github.com/harveyTon/me --tag v0.2.1
```
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/harveyTon/me/main/scripts/install.sh) -- v0.2.1
```
