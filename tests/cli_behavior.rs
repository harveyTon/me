...
fn install_non_interactive_writes_managed_block() {
    ...
    assert!(
        contents.contains("# me-managed: shell=zsh login=full interactive=compact version=v0.3.4")
    );
...
fn uninstall_non_interactive_global_requires_yes() {
    ...
    fs::write(
        &zshrc,
        "# >>> me install >>>\n# me-managed: shell=zsh login=none interactive=compact version=v0.3.4\nme --compact\n# <<< me install <<<\n",
    )
    .unwrap();
...
fn uninstall_non_interactive_yes_removes_all_managed_blocks() {
    ...
    fs::write(
        &bashrc,
        "# >>> me install >>>\n# me-managed: shell=bash login=none interactive=compact version=v0.3.4\nme --compact\n# <<< me install <<<\n",
    )
    .unwrap();
